...
const GlobalSpotlight: React.FC<{
  ...
  useEffect(() => {
    ...
    const handleMouseLeave = () => {
      gridRef.current?.querySelectorAll('.bento-card').forEach(card => (card as HTMLElement).style.setProperty('--glow-intensity', '0'));
...
const MagicBento: React.FC<BentoProps> = ({
  ...
  const cardData: BentoCardProps[] = cards ?? [
    { color: '#060010', title: 'Full Stack Developer', description: 'React · NestJS · TypeScript · PostgreSQL', label: 'Focus' },
    { color: '#060010', title: '4+', description: 'Projects built', label: 'Projects' },
    { color: '#060010', title: 'Always Learning', description: 'Committed to continuous growth and open source', label: '∞' },
    { color: '#060010', title: 'Colombia', description: 'Available worldwide', label: 'CO' },
    { color: '#060010', title: 'Innovation First', description: 'Building scalable, user-centered solutions', label: 'Vision' },
    { color: '#060010', title: 'Open Source', description: 'Learning from the community every day', label: 'Code' },
  ];
  ...
  return (
    <>
      ...
      <div className="bento-section-wrapper w-full" ref={gridRef}>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 w-full">
          {cardData.map((card, index) => {
            const baseClass = `bento-card flex flex-col justify-between relative min-h-[180px] w-full p-5 rounded-2xl border font-light overflow-hidden transition-all duration-300 ${enableBorderGlow ? 'bento-card--border-glow' : ''}`;
            ...
            // Wider cards for first and last
            const spanClass = index === 0 ? 'lg:col-span-2' : index === 2 ? 'lg:col-span-2 lg:row-span-2' : '';

            const content = (
              <>
                <div className="relative text-white/50 text-xs uppercase tracking-widest">{card.label}</div>
                <div className="relative mt-auto">
                  <h3 className={`font-semibold text-white text-base mb-1 ${textAutoHide ? 'text-clamp-1' : ''}`}>
                    {card.title}
                  </h3>
                  <p className={`text-white/40 text-xs leading-relaxed ${textAutoHide ? 'text-clamp-2' : ''}`}>
                    {card.description}
                  </p>
                </div>
              </>
            ...
            return (
              <div key={index} className={`${baseClass} ${spanClass}`} style={cardStyle}>
                {content}
              </div>
            );
          })}
